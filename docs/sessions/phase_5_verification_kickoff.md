...
**같은 세션 후반부 (2026-05-10) — 5.1~5.7 진행됨**.
**현재 commit**: `a3620a1` (Phase 5.7) → 5.8+ 가 다음 세션.
**누적 test**: Phase 4 끝 998 → Phase 5.7 끝 1076 (+78 신규 physics tests).
...
권고 다음 sub-step (가벼움):
- **5.8 ballistic analytic** — `z(t) = z0 - 0.5 g t^2` (RK4 짧은
  시간 통합 후 analytic 과 비교, atol O(g·dt²·N), `velocity_mps`
  kwarg 트랩 주의).
- **5.9 extended target multi-scatterer** — apparent_position 가
  glint 한계 내에 있는지 (RNG seed 고정).
- **5.10 ray_tracing horizon refraction** — 이미 `test_horizon.py`
  있음; `effective_earth_radius_m(k=4/3)` 만 골든으로 보강.
- **5.11 reflection / multipath** — Two-ray path difference vs flat-
  earth approx.

추가 함수 시그니처 트랩:
- `element_power(theta_az_deg, phi_el_deg, kind)` — positional /
  keyword 둘 다 가능, kind 는 `"isotropic"` / `"cos"` 만.
- `zip` 대신 `itertools.pairwise` (RUF007 ruff rule).
...
총 **25 commits** main 직접 push:
- 4.2a~4.12 (12 sub-phase): UI 골격
- ci_log + handoff docs (3)
- 5.1, 5.2, 5.3, 5.4, 5.5, 5.6, 5.7: 검증 프레임워크 7 카테고리

CI: 다수 success 회수, 마지막 몇 개 in_progress (다음 세션에서 회수).
누적 1076 PASS local. 5 contracts KEPT.
...
# 98 PASS (golden_dataset 9 + fmcw 8 + parabolic 11 + atm 13 + forces 6
#  + monopulse 9 + planar element 22 + 기존 20)
PYTHONUTF8=1 PYTHONPATH="$(pwd)/src" "$PY" -m pytest -q
# 1076 PASS total
```

5.8 (ballistic analytic) 부터 빠르게 진입.
