...
**세션 후반부 (2026-05-10~11) — 5.1~5.18 진행됨 (5.15/5.16 skip)**.
**현재 commit**: Phase 5.18 (GNN data association scenario) → 5.19+ 보강
또는 Phase 6 진입.
**누적 test**: Phase 4 끝 998 → Phase 5.18 끝 1185 (+187 신규 tests).
...
| 5.15 | SKIP | `domain/coherence_validator.py` 미구현 (plan/11 § 11.7 정의만). Phase 6+ implementation 이후 추가. |
| 5.16 | SKIP | `domain/simulation_domain.py` 미구현 (plan/11 § 11.11.7 정의만). Phase 6+ implementation 이후 추가. |
...
| 5.18 | TBD | `tests/unit/domain/test_data_association_scenario.py` — close pair → assigned / far pair → gated / two-track-two-detection no-double-assignment + 가까운 짝 winner / two-track-one-detection closer winner / az ±pi wrap 경계 / Mahalanobis = 0 for perfect measurement / noise std + gating threshold rejection / DEFAULT_GATING_THRESHOLD_CHI2 ≈ 14.16. 12 tests. 기존 unit test 3 종 (empty/threshold) 과 중복 회피 — 5.18 은 multi-target Hungarian global optimum. |
...
plan/04 § 4.3 Phase 5 list 에서 **5.1~5.18 완료** (5.15/5.16 skip):
...
- 5.14 ExtendedTarget multi-scatterer + glint
- 5.17 EKF + UKF scenario regression
- 5.18 GNN data association regression

Skip (구현 미존재):
- 5.15 coherence_validator (plan/11 § 11.7 정의만)
- 5.16 simulation_domain.sample_terrain_safe (plan/11 § 11.11.7 정의만)

선택 보강 (기존 test 이미 견고):
- 5.19 `physics/propagation/multipath.py` — `tests/physics/test_two_ray_multipath.py`
  15 tests 존재 (closed-form 기반). golden JSON 재정렬은 선택.
- 5.20 `physics/propagation/ray_tracing.py` horizon — `tests/physics/test_horizon.py`
  23 tests 존재. golden JSON 재정렬은 선택.
...
# 186 PASS (golden 9 + fmcw 8 + parabolic 11 + atm 13 + forces 6
#  + monopulse 9 + planar 22 + ballistic 12 + rcs 14 + cfar 16
#  + extended_target_glint 19 + 기존 47)
...
# 21 신규 + 기존 (tracker scenario 9 + data assoc 12)
PYTHONUTF8=1 PYTHONPATH="$(pwd)/src" "$PY" -m pytest -q
# 1185 PASS total
```

Phase 6 (NN 통합) 또는 5.15/5.16 도메인 코드 구현부터 진입.
