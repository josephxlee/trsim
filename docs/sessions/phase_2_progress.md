...
- 완료: 2.1 / 2.2 / 2.3a / 2.3b / 2.3c
- CI: 6 환경 모두 PASS (마지막 push 기준; 2.3c push 대기 중)
- Phase 0 + 1 + 2 부분 누적 test: ~258
...
| 2.3d | `domain/target.py` | TargetEntity / TargetWaypoint | TBD |
| 2.4 | `physics/dynamics/` (6 모듈) | RigidBodyState / Forces / Solver (RK4) | TBD |
| 2.5 | `physics/atmosphere/` | ISA + rain attenuation | TBD |
| 2.6 | `physics/antenna/` | parabolic / monopulse / beam_pattern | TBD |
...
**우선순위 (의존 그래프 기준)**:
1. **2.3d** Target — placement family 마무리 (의존: 2.3a)
2. **2.5** Atmosphere — 작은 독립 모듈, Octave 가능
3. **2.6** Antenna — 중간 독립, 분석 공식 검증
4. **2.4** Dynamics — 큰 모듈 (6 sub-modules)
