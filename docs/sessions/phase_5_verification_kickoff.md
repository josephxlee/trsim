...
**같은 세션 후반부 (2026-05-10~11) — 5.1~5.14 진행됨**.
**현재 commit**: Phase 5.14 (ExtendedTarget multi-scatterer + glint) →
5.15+ 가 다음 세션.
**누적 test**: Phase 4 끝 998 → Phase 5.14 끝 1164 (+166 신규 tests).
...
| 5.14 | TBD | `tests/physics/test_extended_target_glint.py` + `golden/extended_target_glint.json` — 2-scatterer along-LOS amplitude(1e-6) / amp ratio((R0/R1)²) / centroid(1000.4995) / |sum|(1.2248e-6) / total_signal real+imag bit-for-bit. Skolnik invariants: apparent ∈ scatterer ENU bbox (5 attitude/freq cases) + |sum|≤Σamp triangle ineq + total_rcs_dbsm attitude-invariant (4 cases) + deterministic 반복 + body-x scatterers roll invariant + 대칭 paired scatterers freq sweep glint mean≈0. 19 tests. 기존 Phase 2.7 unit test (`tests/unit/physics/test_extended_target.py`) 와 중복 회피 — 5.14 는 closed-form golden + 회귀 invariant 만. |
...
plan/04 § 4.3 Phase 5 list 에서 **5.1~5.14 완료**:
...
11. `physics/propagation/multipath.py` — Two-ray multipath lobing
    (`tests/physics/test_two_ray_multipath.py` 이미 있음; 골든 패턴
    재정렬은 선택)
12. `physics/propagation/ray_tracing.py` horizon — refraction (4/3
    earth radius). `test_horizon.py` 이미 있음.
...
권고 다음 sub-step (가벼움 → 무거움 순):
- **5.15 Coherence validator** — 6종 검사 동작 (vertical_ref /
  해안선 / 건물 / 해상 / vertical_명시 / SimulationDomain).
- **5.16 simulation_domain.sample_terrain_safe** — Map 안/밖 동작
  일관성.
- **5.17+ EKF/UKF/GNN tracker** — 시나리오 의존, 무거움.

Phase 5.14 후속 후보 (있으면 좋지만 MVP 아님):
- ExtendedTarget Skolnik σ_glint ≈ L/(2√N) RMS 회귀 — 광범위 freq
  + attitude RNG sweep (현재 5.14 는 mean<0.2 만 검증, RMS rule-of-
  thumb 정량 비교는 후속).
...
- 5.1~5.14: 검증 프레임워크 14 카테고리, +166 verification tests
- Phase 5 의 절반 이상 (14/~20 카테고리) 완료

CI: 다수 success 회수, 마지막 몇 개 in_progress (다음 세션에서 회수).
누적 1164 PASS local. 5 contracts KEPT 매 commit.
...
# 27 PASS (clock 9 + frame_boundary 6 + frame_profiler 12)
PYTHONUTF8=1 PYTHONPATH="$(pwd)/src" "$PY" -m pytest -q
# 1164 PASS total
```

5.15 (Coherence validator 6 검사) 부터 빠르게 진입.
